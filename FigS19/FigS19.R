...
Input_re_list<-list(M_mbayNorm,R_mbayNorm,T_mbayNorm,M_modebayNorm,R_modebayNorm,T_modebayNorm,apply(M_abayNorm,1,median),R_abayNorm,T_abayNorm)
...
library(ggplot2)
textsize<-12
BAR_OUT<-ggplot(data=BAR_DAT, aes(x=BAR_DAT[,2], y=BAR_DAT[,1], fill=BAR_DAT[,3])) +
    geom_bar(stat="identity", position = position_dodge(0.9),width=0.9)+
    geom_text(aes(label=BAR_DAT[,1]), vjust=0, color="black", position = position_dodge(0.9), size=2)+
    labs(fill = "Gene expression group",y='Number of detected DE genes',x='DE method_type of output from bayNorm')+ggtitle("") +
    scale_fill_brewer(palette="Paired")+
...
BAR_DAT_simH1<-foreach(i=1:length(sreg),.combine=rbind)%:%
    foreach(j=1:length(INPUT_LIST),.combine=rbind)%do%{
        qq<-length(intersect(names(which(INPUT_LIST[[j]]<thres)),names(sreg[[i]])))
        qq2<-c(qq,names(INPUT_LIST)[j],i)
        return(qq2)
...
BAR_DAT_simH1<-as.data.frame(BAR_DAT_simH1)
colnames(BAR_DAT_simH1)<-c('Number of detected DE genes','Normalization methods','Gene expression group')
BAR_DAT_simH1[,1]<-as.numeric(as.character(BAR_DAT_simH1[,1]))
BAR_DAT_simH1[,2]<-factor(BAR_DAT_simH1[,2],levels=unique(BAR_DAT_simH1[,2]))
BAR_DAT_simH1[,3]<-factor(BAR_DAT_simH1[,3],levels=unique(BAR_DAT_simH1[,3]))
...
library(ggplot2)
textsize<-12
BAR_OUT_simH1<-ggplot(data=BAR_DAT_simH1, aes(x=BAR_DAT_simH1[,2], y=BAR_DAT_simH1[,1], fill=BAR_DAT_simH1[,3])) +
    geom_bar(stat="identity", position = position_dodge(0.9),width=0.9)+
    geom_text(aes(label=BAR_DAT_simH1[,1]), vjust=0, color="black", position = position_dodge(0.9), size=2)+
    labs(fill = "Gene expression group",y='Number of detected DE genes',x='DE method_type of output from bayNorm')+ggtitle("") +
    scale_fill_brewer(palette="Paired")+
    theme(legend.text = element_text(size = textsize),legend.title  = element_text(size = textsize),plot.title = element_text(size = textsize),axis.title = element_text(size = textsize),panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
          panel.background = element_blank(), axis.line = element_line(colour = "black"),plot.subtitle = element_text(size = textsize),plot.caption =  element_text(size = textsize),axis.text=element_text(size=textsize) ,legend.key.size = unit(1,"line"),legend.position ='top')
