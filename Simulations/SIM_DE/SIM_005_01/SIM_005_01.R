...
#set mean BETA to be 5% and 10% in each group respectively
Est_params@MeanBeta<-c(0.05,0.1)
...
mbayNorm_out<-bayNorm_sup(Data<-RAW_DAT,BETA_vec<-c(BETA_SCRAN_1,BETA_SCRAN_2),Conditions=CONDITION,mean_version = T,S=1000,PRIORS = bayNorm_out$PRIORS_LIST)
gr<-table(CONDITION)
...
save.image("E:/RNAseqProject/SIMULATION/SIM_005_01/SIM_005_01.RData")
...
M_scnorm<-SCnorm_runMAST(Data=scnorm_out,NumCells = c(100,100))

#save(scnorm_out_ori,scnorm_out,M_scnorm,file="E:/RNAseqProject/SIMULATION/SIM_005_01/SIM_005_01_SCnorm.RData")
...
#save(saver_temp1,saver_temp2,saver_out,M_saver,M_saver10,saver_array1,saver_array2,file="E:/RNAseqProject/SIMULATION/SIM_005_01/SIM_005_01_saver_BETA.RData")


#scImpute############
library(scImpute)
scimpute_fun(Data=RAW_DAT,Data_name='DE_sim_005_01',Kcluster=2)
scImpute_out<-readRDS("E:/RNAseqProject/SIMULATION/SIM_005_01/DE_sim_005_01scimpute_count.rds")
gr<-table(CONDITION)
...
magic_out<-as.matrix(magic_out)
magic_out<-t(magic_out)
rownames(magic_out)<-rownames(RAW_DAT)
colnames(magic_out)<-colnames(RAW_DAT)
...
save(GG_bayNorm_out,GG_mbayNorm_out,M_GG_bayNorm_out,M_GG_mbayNorm_out,auc_GG_bayNorm_out,auc_GG_mbayNorm_out,file="E:/RNAseqProject/SIMULATION/DE_GG_explore/GG_SIM_005_01.RData")
