...
for(i in 1:length(Fig3Gene)){
  ...
  if(Fig3Gene[i]!='Pou5f1'){
    ...
    for(j in 1:length(Grun_2i_norm_list)){
      ...
      if(length(intersect(Fig3Gene[i],colnames(Grun_2i_norm_list[[j]])))==1){
        ...
        boot_2imat_smFISH[,i]<-foreach(qq=1:1000,.combine=c)%do%{
          qq_temp<-Gini(sample(Grun_2i_norm_list[[j]][,asd],replace=T))
          return(qq_temp)
        }
      }
    }
    ...
    boot_2imat_smFISH[,9]<-foreach(qq=1:1000,.combine=c)%do%{
      qq_temp<-Gini(sample(useeeee,replace=T))
      return(qq_temp)
    }
  }
}
    ...
    for(j in 1:length(Grun_S_norm_list)){
      ...
      if(length(intersect(Fig3Gene[i],colnames(Grun_S_norm_list[[j]])))==1){
        ...
        boot_serummat_smFISH[,i]<-foreach(qq=1:1000,.combine=c)%do%{
          qq_temp<-Gini(sample(Grun_S_norm_list[[j]][,asd],replace=T))
          return(qq_temp)
        }
      }
    }
...
boot_2imat_bayNorm<-foreach(i=1:9,.combine=cbind)%:%
  foreach(j=1:1000,.combine=c)%do%{
    ...
    qqtemp<-Gini(sample(bayNorm_SC_2i$Bay_array[qqind,,],replace=T))
    return(qqtemp)
  }
...
boot_serummat_bayNorm<-foreach(i=1:9,.combine=cbind)%:%
  foreach(j=1:1000,.combine=c)%do%{
    ...
    qqtemp<-Gini(sample(bayNorm_SC_serum$Bay_array[qqind,,],replace=T))
    return(qqtemp)
  }
...
boot_2imat_scnorm<-foreach(i=1:9,.combine=cbind)%:%
  foreach(j=1:1000,.combine=c)%do%{
    ...
    qqtemp<-Gini(sample(SCnorm_2i[qqind,],replace=T))
    return(qqtemp)
  }
...
boot_serummat_scnorm<-foreach(i=1:9,.combine=cbind)%:%
  foreach(j=1:1000,.combine=c)%do%{
    ...
    qqtemp<-Gini(sample(SCnorm_serum[qqind,],replace=T))
    return(qqtemp)
  }
...
boot_2imat_scran<-foreach(i=1:9,.combine=cbind)%:%
  foreach(j=1:1000,.combine=c)%do%{
    ...
    qqtemp<-Gini(sample(scran_2i[qqind,],replace=T))
    return(qqtemp)
  }
...
boot_serummat_scran<-foreach(i=1:9,.combine=cbind)%:%
  foreach(j=1:1000,.combine=c)%do%{
    ...
    qqtemp<-Gini(sample(scran_serum[qqind,],replace=T))
    return(qqtemp)
  }
...
boot_2imat_RPM<-foreach(i=1:9,.combine=cbind)%:%
  foreach(j=1:1000,.combine=c)%do%{
    ...
    qqtemp<-Gini(sample(RPM_2i[qqind,],replace=T))
    return(qqtemp)
  }
...
boot_serummat_RPM<-foreach(i=1:9,.combine=cbind)%:%
  foreach(j=1:1000,.combine=c)%do%{
    ...
    qqtemp<-Gini(sample(RPM_serum[qqind,],replace=T))
    return(qqtemp)
  }
...
boot_2imat_TMM<-foreach(i=1:9,.combine=cbind)%:%
  foreach(j=1:1000,.combine=c)%do%{
    ...
    qqtemp<-Gini(sample(TMM_2i[qqind,],replace=T))
    return(qqtemp)
  }
...
boot_serummat_TMM<-foreach(i=1:9,.combine=cbind)%:%
  foreach(j=1:1000,.combine=c)%do%{
    ...
    qqtemp<-Gini(sample(TMM_serum[qqind,],replace=T))
    return(qqtemp)
  }
...
boot_2imat_DESeq<-foreach(i=1:9,.combine=cbind)%:%
  foreach(j=1:1000,.combine=c)%do%{
    ...
    qqtemp<-Gini(sample(DESeq_2i[qqind,],replace=T))
    return(qqtemp)
  }
...
boot_serummat_DESeq<-foreach(i=1:9,.combine=cbind)%:%
  foreach(j=1:1000,.combine=c)%do%{
    ...
    qqtemp<-Gini(sample(DESeq_serum[qqind,],replace=T))
    return(qqtemp)
  }
...
boot_2imat_SAVER<-foreach(i=1:9,.combine=cbind)%:%
  foreach(j=1:1000,.combine=c)%do%{
    ...
    qqtemp<-Gini(sample(as.vector(SAVER_SC_2i_array[qqind,,]),replace=T))
    return(qqtemp)
  }
...
boot_serummat_SAVER<-foreach(i=1:9,.combine=cbind)%:%
  foreach(j=1:1000,.combine=c)%do%{
    ...
    qqtemp<-Gini(sample(as.vector(SAVER_SC_serum_array[qqind,,]),replace=T))
    return(qqtemp)
  }
...
boot_2imat_RAW_BETA<-foreach(i=1:9,.combine=cbind)%:%
  foreach(j=1:1000,.combine=c)%do%{
    ...
    qqtemp<-Gini(sample(RAW_BETA_2i[qqind,],replace=T))
    return(qqtemp)
  }
...
boot_serummat_RAW_BETA<-foreach(i=1:9,.combine=cbind)%:%
  foreach(j=1:1000,.combine=c)%do%{
    ...
    qqtemp<-Gini(sample(RAW_BETA_serum[qqind,],replace=T))
    return(qqtemp)
  }
...
boot_2imat_scImpute<-foreach(i=1:9,.combine=cbind)%:%
  foreach(j=1:1000,.combine=c)%do%{
    ...
    qqtemp<-Gini(sample(scImpute_SC_2i[qqind,],replace=T))
    return(qqtemp)
  }
...
boot_serummat_scImpute<-foreach(i=1:9,.combine=cbind)%:%
  foreach(j=1:1000,.combine=c)%do%{
    ...
    qqtemp<-Gini(sample(scImpute_SC_serum[qqind,],replace=T))
    return(qqtemp)
  }
...
boot_2imat_MAGIC<-foreach(i=1:9,.combine=cbind)%:%
  foreach(j=1:1000,.combine=c)%do%{
    ...
    qqtemp<-Gini(sample(MAGIC_2i[qqind,],replace=T))
    return(qqtemp)
  }
...
boot_serummat_MAGIC<-foreach(i=1:9,.combine=cbind)%:%
  foreach(j=1:1000,.combine=c)%do%{
    ...
    qqtemp<-Gini(sample(MAGIC_serum[qqind,],replace=T))
    return(qqtemp)
  }
...
boot_2imat_DCA<-foreach(i=1:9,.combine=cbind)%:%
  foreach(j=1:1000,.combine=c)%do%{
    ...
    qqtemp<-Gini(sample(DCA_2i[qqind,],replace=T))
    return(qqtemp)
  }
...
boot_serummat_DCA<-foreach(i=1:9,.combine=cbind)%:%
  foreach(j=1:1000,.combine=c)%do%{
    ...
    qqtemp<-Gini(sample(DCA_serum[qqind,],replace=T))
    return(qqtemp)
  }
...
#Store into list#####
STORE_BOOTGINI_list<-list(24)
qqq<-ls()
qqq2<-grep(ls(),pattern='boot_')
for(i in 1:24){
  STORE_BOOTGINI_list[[i]]<-get(qqq[qqq2[i]])
}

names(STORE_BOOTGINI_list)<-qqq[qqq2]

#new MAGIC+default saver +DCA
save(STORE_BOOTGINI_list,file="E:/RNAseqProject/NEWPROJECT_PAPERS/Validation of noise models for single-cell transcriptomics/Grun_smFISH_meanBETA/Bootstrapping_Gini_V3tr_default_divmeanbeta.RData")
